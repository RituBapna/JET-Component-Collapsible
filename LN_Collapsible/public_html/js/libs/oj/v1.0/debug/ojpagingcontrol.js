  ...
  oj.__registerWidget("oj.ojPagingControl", $['oj']['baseComponent'],
    {
      ...
      'nextPage': function()
      {
        ...
        if (data != null)
        {
          ...
          {
            var page = this._getCurrentPage();
            if (page + 1 > this._getTotalPages()) {
                // Guard against walking off the end
                page = this._getTotalPages()-1;
            }
            this._startIndex = this._getStartIndexForPage(page + 1);
            return data.fetch({'startIndex': this._startIndex});
          }
      ...
      _draw: function()
      {
        ...
        var self = this;
        setTimeout(function() {
          if (self._getData() != null &&
              self._getData().size() < options['pageSize'] &&
              (self._getData().totalSize() < 0 || self._getData().totalSize() >= self._getData().size()))
          {
            self._getData().setPageSize(options['pageSize']);
            self._getData().fetch({'startIndex': self._startIndex, 'fetchType': 'init'});
          }
        }, 0);
      },
      /**
      ...
      _getItemRangeText: function()
      {
        ...
        if (data != null)
        {
          ...
          if (data.totalSize() != -1)
          {
            ...
            itemRangeText = this.getTranslatedString(this._BUNDLE_KEY._MSG_ITEM_RANGE, {'pageFrom': this._startIndex + 1, 'pageTo': pageTo, 'pageTotal': data.totalSize()});
          }
          else
          {
            itemRangeText = this.getTranslatedString(this._BUNDLE_KEY._MSG_ITEM_RANGE_UNKNOWN, {'pageFrom': this._startIndex + 1, 'pageTo': pageTo});
          }
        }
      ...
      _refreshPagingControlNavArrows: function()
      {
        ...
        if (pagingControlNavNext && pagingControlNavNext.length > 0)
        {
          ...
          if (this._getCurrentPage() == this._getTotalPages() || this._getTotalPages() === 0 ||
              (this._getTotalPages() < 0 && this._getData() != null && this._getData().size() === 0))
          {
      ...
      _createPagingControlNav: function()
      {
            ...
            $.inArray(this._PAGE_OPTION_LAYOUT._INPUT, pageOptionLayout) != -1)
        {
          ...
          {
            if (this._getData() != null && this._getData().size() > 0)
            {
              maxPageVal = this._getCurrentPage() + 1;
            }
            else
            {
              maxPageVal = this._getCurrentPage();
            }
          }
