...
{
    var rowIndex, columnIndex, rowKey, columnKey, atPromise;
...
{
    var rowKey, columnKey, columnIndex;
    rowKey = keys['row']; 
    ...
    return oj.Object.__getPromise(function(resolve, reject) {
        self.collection.indexOf(rowKey, {deferred:true}).then(function(rowIndex){
            if (self.columns == null)
            {
                self.collection.first(1, {'deferred':true}).then(function(model){self._setupColumns(model);}.bind(self));
            } 
            columnIndex = self.columns.indexOf(columnKey);
            resolve({"row": rowIndex == null ? -1:rowIndex, "column": columnIndex == null ? -1:columnIndex});
        }.bind(self));
    });
};
...
oj.CollectionDataGridDataSource.prototype.sort = function(criteria, callbacks, callbackObjects)
{
    ...
    if (axis === "column") {
        ...
        if (this.collection.fetchSize > -1 && this.collection.__isVirtual())
        {
