...
oj.FlattenedTreeRowSet._init = function(rowSet, data, options) 
{
  ...
  rowSet._data.insertRows = function(insertAtIndex, insertAtKey, nodeSet)
  {
    ...
    if (rowSet._pageSize)
    {
      rowSet._updateContext();
      setTimeout(function(){
        oj.FlattenedTreeRowSet.superclass._handleEvent.call(rowSet, oj.RowSet.EventType['SYNC'], {'refresh': true});
      }, 0);
    }
  };
...
oj.FlattenedTreeRowSet.prototype._updateContext = function()
{
  var i;
...
oj.PagingTableDataSource.prototype._handleSyncEvent = function(event, eventHandler)
{
  ...
  setTimeout(function()
    {
      if (self._fetchType == 'next' || (event != null && event['fetchType'] == 'next'))
      {
  ...
  oj.__registerWidget("oj.ojTable", $['oj']['baseComponent'],
    {
      ...
      _ComponentCreate : function ()
      {
        ...
        setInterval(
          function()
          {
            ...
            {
              self._setSelection(self.options['selection']);
      ...
      _refresh: function(immediate)
      {
        ...
        if (immediate)
        {
          this._refreshAll(immediate, true);
        }
        else
        {
          var self = this;
          setTimeout(function()
          {
            self._refreshAll(immediate, true);
          }, 0);
        }
      },
      /**
      ...
      _setOption: function(key, value)
      {
        ...
        if (key == 'selection')
        {
          ...
          this._fireOptionChange('selection', existingSelection, value, null); 
        }
        else if (key == 'currentRow')
        {
          this._setCurrentRow(value, null);
        }
      },
      ...
      _handleDataSort: function(event)
      {
        if (!this._dataFetching) {
            ...
            this._fireOptionChange('selection', existingSelection, this['options']['selection'], null); 
            // set the current row
      ...
      /**
       * @param {boolean} immediate  refresh immediately
       * @param {boolean} executeRowOperations  execute row operations
       * @private
       */
      _refreshAll: function(immediate, executeRowOperations)
      {
        this._refreshTableNoDataMessage();
        if (executeRowOperations)
        {
          this._executeQueuedTableBodyRowOperations();
        }
      ...
      _setHeaderColumnSelection: function(columnIdx, selected, element, event, updateSelection)
      {
          ...
          this._getColumnSelectionMode() == this._OPTION_SELECTION_MODES._MULTIPLE)
        {
          ...
          if (updateSelection)
          {
            ...
            this._fireOptionChange('selection', existingSelection, selection, null);
          }
        }
      },
      /**
...
oj.TableDomUtils.prototype.clearCachedDom = function()
{
  ...
  this._tableElementDimensions = null;
 ...
 */
oj.TableDomUtils.prototype.getTableDimensions = function()
{
  var table = this.getTable();

  // hide all children so it does affect the style
  var i;
  var tableChildrenDisplayStyle = [];
  
  for (i = 0; i < table.children().length; i++)
  {
    tableChildrenDisplayStyle[i] = $(table.children()[i]).css('display').toString();
    $(table.children()[i]).css('display', 'none');
  }
  table.removeClass(oj.TableDomUtils.CSS_CLASSES._TABLE_ELEMENT_CLASS);
  table.css('display', 'inline-block');
  var dimensions = {height: table.height(), width: table.width()};
  for (i = 0; i < table.children().length; i++)
  {
    $(table.children()[i]).css('display', tableChildrenDisplayStyle[i]);
  }
  table.addClass(oj.TableDomUtils.CSS_CLASSES._TABLE_ELEMENT_CLASS);
  table.css('display', '');
...
oj.TableDomUtils.prototype.renderDelayedTableBodyCell = function(rowIdx, columnIdx)
{
  ...
  if (delayedCell != null)
  {
    var cellColumnContent = delayedCell.cellRenderer({'cellContext': delayedCell.cellContext,
      ...
      'row': $.extend({}, delayedCell.row.attributes)});
...
oj.TableDomUtils.prototype.renderDelayedTableBodyRow = function(rowIdx)
{
  ...
  if (delayedRow != null)
  {
    var rowContent = delayedRow.rowRenderer({'rowContext': delayedRow.rowContext, 
                                             'row': $.extend({}, delayedRow.row.attributes)});
 ...
 oj.TableDomUtils.prototype._refreshTableDimensions = function(width, height)
{
  ...
  if (width > 0 && height > 0 && 
      this._tableWidthConstrained === false && this._tableHeightConstrained === false)
  {
  ...
  if (this._tableContainerDimensions != null)
  {
    ...
    {
      this._tableDimensions['width'] = width;
    }
    if (height > 0 && this._tableHeightConstrained)
    {
      this._tableDimensions['height'] = height;
    }
  }
  else
  {
    ...
    // Denotes whether the table container or table dimensions are constrained
    // such as by stretching or absolute dimensions
    this._tableHeightConstrained = true;
    this._tableWidthConstrained = true;

    if (this._tableContainerDimensions['height'] == 0 && this._tableElementDimensions['height'] == 0)
    {
    ...
    else
    {
      ...
      {
        this._tableDimensions['height'] = this._tableContainerDimensions['height'];
      }
      else
      {
        this._tableDimensions['height'] = this._tableElementDimensions['height'];
        tableContainer.css('height', this._tableDimensions['height'] + 'px');
      }
    }
    if (this._tableContainerDimensions['width'] == 0 && this._tableElementDimensions['width'] == 0)
    {
    ...
    else
    {
      ...
      {
        this._tableDimensions['width'] = this._tableContainerDimensions['width'];
      }
      else
      {
        this._tableDimensions['width'] = this._tableElementDimensions['width'];
        tableContainer.css('width', this._tableDimensions['width'] + 'px');
      }
    }
  }
  ...
  if (!this.isDivScroller())
  {
    minHeight = parseInt(table.css('minHeight'), 10);
    minWidth = parseInt(table.css('minWidth'), 10);
  }
  else if (table[0]['currentStyle'])
  {
    // use currentStyle for IE9
    minHeight = parseInt(table[0]['currentStyle']['minHeight'], 10);
    minWidth = parseInt(table[0]['currentStyle']['minWidth'], 10);
  }
  ...
  if (!isTableHeightScrolled && !isTableWidthScrolled &&
    !this._tableHeightConstrained && !this._tableWidthConstrained &&
    !isTableMinHeightSet)
  {
  ...
  if (!this.isDivScroller())
  {
    ...
    {
      tableBody.css('width', this._tableDimensions['width'] - this._tableBorderWidth);
    }
    
    // if we don't use fallback scrolling then size the table body
    // to fit in the height
    var tableBodyHeight = this._tableDimensions['height'] - tableHeaderHeight - tableFooterHeight - captionHeight - this._tableBorderHeight;
    ...
    // setting the tbody height may have caused scrollbars to appear.
    if (scrollbarWidth > 0)
    {
      isTableWidthScrolled = true;
    }
    ...
    if (isTableWidthScrolled)
    {
      if (data != null && data.size() > 0)
      {
        ...
        if (tableHeader != null)
        {
          tableHeader.css('width', this._tableDimensions['width'] - this._tableBorderWidth);
        }
      }
      else
      {
  ...
  if (this.isDivScroller())
  {
    ...
    if (this._tableWidthConstrained)
    {
      tableDivScroller.css('width', this._tableDimensions['width']);
    }
    if (this._tableHeightConstrained)
    {
      tableDivScroller.css('height', this._tableDimensions['height'] - tableHeaderHeight - tableFooterHeight);
    }
