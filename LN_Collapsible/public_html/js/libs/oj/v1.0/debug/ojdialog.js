...
(function() {
    ...
    * (e.g., do not rely on relative positioning of dialog DOM elements).
    *
    * <h3 id="focus-section">
    *   Focus
...
/*
	    ...
	    this._AcssHeight = this.element.css('height');
	    this._AcssWidth = this.element.css('width');

	    this._elem =  this.element[0];
	    this._elemStyle = this.element[0].style;
	    this._aeHeight = this.element[0].style.height;
	    this._aeWidth = this.element[0].style.width;

	    this._AcssHeight = this.uiDialog.css("height");
	    this._AcssWidth = this.uiDialog.css("width");
*/
   ...
   // * @memberof oj.ojDialog
	...
	_trackingInstances: function() {
		var instances = this.document.data( "oj-dialog-instances" );
		if ( !instances ) {
			instances = [];
			this.document.data( "oj-dialog-instances", instances );
		}
		return instances;
	},
...
*/
	...
	_createTitlebar: function() {
	    ...
	    // 
	    // Add an additional header class if we do not display an icon.
	    // This is necessary for centering the title and adding padding.
	    // 
	    if (this.options.cancelBehavior === "icon")
		headerClasses = "oj-dialog-header oj-dialog-header-with-close-icon oj-helper-clearfix";
	    else
		headerClasses = "oj-dialog-header oj-dialog-header-without-close-icon oj-helper-clearfix";
	...
	_makeResizable: function() {
		...
		//
		// These variables are needed in order to stop the resizing
		// once the limits have been reached.
		// (Otherwise, we get strange embedded scrollbars)
		// 
