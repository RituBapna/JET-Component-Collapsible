...
{
  var self = this, latestContent = this._buildPopupHtml(), domNode;
 ...
 * @ignore
 */ 
var _sValidationMode = {FULL : 1, VALIDATORS_ONLY : 2, REQUIRED_VALIDATOR_ONLY : 3};
 ...
 * </h3>
 * An editable component runs validation (normal or deferred) at various times based on the actions 
 * performed on it (both by end-user and author), and the state it was in when the actions occurred. 
 * These factors also determine whether validation errors/messages are shown to the end-user 
 * immediately or are deferred. The following sections highlight the different types of validation 
 * that are run and how messages get displayed.
 * 
 ...
 * <ul>
 * <li>When value changes as a result of user interaction, all messages are cleared - 
 * <code class="prettyprint">messagesShown</code>, <code class="prettyprint">messagesHidden</code> 
 * and <code class="prettyprint">messagesCustom</code> options, and full validation is run. 
 * The value is parsed, then validated and finally set on the option using the following steps.
 * <ol> 
 * <li>All messages options are cleared, including custom messages added by the app. </li>
 ...
 * </h4>
 * Deferred validation is run in the following cases on the component value, using the required 
 * validator and validation errors are not shown to user immediately, i.e., are hidden.
 * <ul>
 *  <li>When a component is created, validators that participate in deferred validation are run and  
 *  no messages options are cleared prior to running validation.</li>  
...
oj.__registerWidget('oj.editableValue', $['oj']['baseComponent'], 
{
  ...
  options: 
  {
     ...
     * <ul>
     * <li>when component is initialized as disabled <code class="prettyprint">{value: null, required:true, disabled: true}</code>, 
     * then deferred validation is skipped.</li>
     * <li>if disabled component is enabled, 
     *  <ul>
     ...
     *  </ul>
     * </li>
     * <li>if enabled component is disabled then no validation is run and the component appears 
     * disabled.</li>
     * </ul>
     * </p>
     * 
    ...
    /**
     * Specifies the display options for auxilliary content associated with the component that 
     * determines where it gets displayed in relation to the component. It takes an Object literal 
     * containing the properties listed in the table below.
     * 
     * <p>
     * The types of messaging related content include <code class="prettyprint">messages</code>, 
     ...
     * <code class="prettyprint">oj.Components#setDefaultOptions</code> method.<br/>
     * </p>
     * <p>
     * When displayOptions changes due to programmatic intervention, the component updates its 
     * display to reflect the preference. For example, if 'title' property goes from 'notewindow' to 
     * 'none' then it no longer shows in the notewindow.
     * </p>
     * 
     ...
     * 
     * @example <caption>Initialize component and override default for converterHint using <code class="prettyprint">displayOptions</code>:</caption>
     ...
     * 
     * @example <caption>Set a message using the <code class="prettyprint">messagesCustom</code> option:</caption>
     * var msgs = [];
     ...
     * deferred validation. These messages are not shown to the end-user by default, but page author 
     * can show hidden messages using the {@link #showMessages} method. Each message in the array is 
     ...
     * This is a read-only option, i.e., page authors cannot set or change it directly. Each message 
     * in the array is either an instance of oj.Message or an object that duck types it. See 
     * {@link oj.Message} for details. <br/>
     * <p>
    ...
    /** 
     * Whether the component is required or optional. When required is set, an implicit required 
     * validator is created using the validator factory - 
     ...
     * the component clears messages and runs validation, based on the current state it's in. </br>
     * 
   ...
   * 
   * - triggers an optionChange and does writeback when necessary. <br/>
...
oj.__registerWidget("oj._ojLabel", $['oj']['baseComponent'],
{
  ...
  */
  ...
  _drawOnCreate : function ()
  {
    ...
    // TODO: we need to make sure there IS a describedById!!
  ...
  _createDescribedBySpan : function ()
  {
    ...
    var describedByHtml = "<span id='" + this.options.describedById + "'></span>";
    return ojLabelGroupDom.prepend(describedByHtml).children().first();
  },
  /**
  ...
  _helpHtml : function (helpDef, source)
  {
    ...
    if (source)
    {
      var helpHtml = "<a href='" + source + "' target='_blank' class='oj-label-help-icon-anchor'><span class=";
  ...
  _refreshHelp : function ()
  {
    ...
    var helpIconSpan = this.uiLabel.find(".oj-label-help-icon");
    if(helpIconSpan.length === 1)
    {
      // remove the <a> around the icon and the icon.
      helpIconSpan.parent().remove(); 
    }
    var needsDescribedBySpan = this.options.help['source'];
    
    // insert describedBySpan if it isn't there AND it is needed
    var describedBySpan = this.uiLabel.find(describedById);
