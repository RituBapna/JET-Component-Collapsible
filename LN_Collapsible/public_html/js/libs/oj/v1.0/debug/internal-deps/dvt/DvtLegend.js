...
DvtLegendRenderer.render = function(legend, availSpace) {
  ...
  var titleDim = DvtLegendRenderer._renderTitle(legend, container, options['title'], legendSpace, null);
  ...
  for (var i = 0; i < titles.length; i++) {
    // Position the titles according to the alignment after we know the total dims
    var titleHalign = titles[i].getHorizAlignment();
    if (titleHalign == DvtOutputText.H_ALIGN_LEFT)
      titles[i].setX(totalDim.x);
    else if (titleHalign == DvtOutputText.H_ALIGN_CENTER)
      titles[i].setX(totalDim.x + totalDim.w / 2);
    else if (titleHalign == DvtOutputText.H_ALIGN_RIGHT)
      titles[i].setX(totalDim.x + totalDim.w);
  }
...
DvtLegendRenderer._renderTitle = function(legend, container, titleStr, availSpace, section, isInline) {
  ...
  if (!titleStr)
    return new DvtRectangle(isRTL ? availSpace.x + availSpace.w : availSpace.x, availSpace.y, 0, 0);

  // Create the title object and add to legend
  var title = new DvtOutputText(container.getCtx(), titleStr, isRTL ? availSpace.x + availSpace.w : availSpace.x, availSpace.y);
  ...
  if (DvtTextUtils.fitText(title, availSpace.w, availSpace.h, container)) {
    ...
    if (isRTL) // align first to get the dims for preferred size
      title.alignRight();
    ...
    if (!options['isLayout']) {
      if (!isInline) {
        ...
        if (titleHalign == 'center')
          title.alignCenter();
        else if (titleHalign == 'end')
          isRTL ? title.alignLeft() : title.alignRight();
        legend.__registerTitle(title); // x positioning will be done later after we know the total width of the legend content
      }
    }
    else {
...
DvtLegendRenderer._renderSections = function(legend, container, availSpace) {
  ...
  var gapHeight = DvtLegendDefaults.getGapSize(options, options['layout']['sectionGapHeight']);
  var gapWidth = DvtLegendDefaults.getGapSize(options, options['layout']['sectionGapWidth']);
  ...
  for (i = 0; i < legendSections.length; i++) {
    if (isHoriz) { // horizontal legend
      ...
      sectionDim = DvtLegendRenderer._renderVerticalSection(legend, container, legendSections[i], i, availSpace, rowHeight, options['scrolling'] != 'off');
      availSpace.y += sectionDim.h + gapHeight;
      availSpace.h -= sectionDim.h + gapHeight;
    }
 ...
 * @param {boolean} isScrollable Whether the section is scrollable.
 * @return {DvtRectangle} The total dimension of the section.
 * @private
 */
DvtLegendRenderer._renderVerticalSection = function(legend, container, section, sectionIndex, availSpace, rowHeight, isScrollable) {
  if (!section)
    return;

  var options = legend.__getOptions();
  var symbolGap = DvtLegendDefaults.getGapSize(options, options['layout']['symbolGapWidth']);
  ...
  var titleDim = DvtLegendRenderer._renderTitle(legend, container, section['title'], sectionSpace, section);
  ...
  var legendInfo = DvtLegendRenderer._calcColumns(legend, sectionSpace, rowHeight, section['items'], isScrollable);
...
DvtLegendRenderer._renderHorizontalSection = function(legend, container, section, sectionIndex, availSpace, rowHeight) {
  ...
  var titleDim = DvtLegendRenderer._renderTitle(legend, container, section['title'], availSpace, section, true);
  if (!section['items'])
    return titleDim;
 ...
 * @param {boolean} isScrollable True if scrolling is allowed.
 * @return {object} Map containing width, rows and columns in the legend.
 * @private
 */
DvtLegendRenderer._calcColumns = function(legend, availSpace, rowHeight, items, isScrollable) {
  var options = legend.__getOptions();
...
DvtLegendRenderer._getRowHeight = function(legend) {
  ...
  var symbolHeight = options['symbolHeight'] + DvtLegendDefaults.getGapSize(options, options['layout']['symbolGapHeight']);
