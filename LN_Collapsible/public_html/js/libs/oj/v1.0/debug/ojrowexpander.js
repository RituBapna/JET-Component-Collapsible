...
oj.__registerWidget('oj.ojRowExpander', $['oj']['baseComponent'],
{
    ...
    {
        var spacer, remainder, i,depth;
        //0 index the depth for style purposes
        depth = this.depth-1;
        if (depth < this.constants.MAX_STYLE_DEPTH)
        {
            spacer = $(document.createElement('span')).addClass(this.classNames['indent']).addClass(this.classNames['depth'+depth]);
        ...
        else
        {
            ...
            {
                spacer = $(document.createElement('span')).addClass(this.classNames['indent']).addClass(this.classNames['depth'+this.constants.MAX_STYLE_DEPTH]);
                this.element.append(spacer);
            }
            remainder = (depth % this.constants.MAX_STYLE_DEPTH);
            if (remainder < this.constants.MAX_STYLE_DEPTH)
            {
                spacer = $(document.createElement('span')).addClass(this.classNames['indent']).addClass(this.classNames['depth'+remainder]);           
                this.element.append(spacer);            
            }
        }
    },
