...
(function () 
{
  ...
  */
  ...
  /**
    ...
    *  @private
    */
  var  _aEvNames = ["optionChange", "hover", "dehover", "expand", "collapse", "loaded",
                    "move", "remove", "rename", "refresh", "expandAll", "collapseAll",
                    "destroy", "create", "before", "remove", "cut", "copy", "paste"] ;
...
/**
  ...
  * <table class="keyboard-table">
  *   <thead>
  *      <tr><th style="min-width:10em">Class</th><th>Use</th></tr>
  *   </thead>
  *   <tbody>
  *     <tr><td>.oj-tree-transition</td>
  *         <td>Applies a duration for the expand and collapse animation of a parent node. Refer to SASS variable <code class="prettyprint">$treeAnimationDuration</code> 
  *                in <code class="prettyprint">oj-alta.css</code> or <code class="prettyprint">oj-jq.css</code>.  To
  *                turn off animation, the application could apply a style like <code class="prettyprint">.oj-tree-transition {transition: 0s}</code>.</td>
  *     </tr>
  *   </tbody>
  *  </table>
  * <br>
  ...
  */
  oj.__registerWidget("oj.ojTree", $['oj']['baseComponent'], 
  {
     ...
     {
        // Get css value $treeAnimationDuration.  We need this because if the value is zero and
        // we start a transition, browsers do not apparently fire the "transitionend" event and
        // so we don't get to emit the  collapse event.
    ...
    /**
     ...
     _slide : function(node, bSlideUp)
     {
        ...
        node.on("transitionend", $.proxy(this._transitionEndHandler, this)) ;
     ...
     _transitionEndHandler : function (e)
     {
        ...
        node.off("transitionend", this._transitionEndHandler) ;
...
//       return node.nodeValue;
     ...
     _setOption: function (key, newval, flags)
     {
        ...
        if (key === "selection") {
          prev  = this.options[key].slice(0) ;     // make a copy for the optionChange event
          this._super(key, newval, flags);
          this._handleSelectionOptionChange() ;
          this._fireOptionChangeEvent(key, prev, this.options["selection"], null) ;
          return ;
        }
...
//      this._emitEvent({});
     ...
     {
        var sels   = this.options["selection"].slice(0) ;
        ...
        this.options["selection"] = newvals ;                         // update the options array.
...
//       this._emitEvent({ "obj" : obj }, "clean_node");
      ...
      _create_node :  function (obj, position, js, callback, is_loaded)
      {
         ...
         $.each(js["data"], function (i, m) {
             ...
             tmp.append(sp) ;
             tmp.prepend("<ins class='oj-tree-icon oj-tree-node-icon oj-component-icon'>&#160;</ins>");
      ...
      _expand : function (obj, callback, skipAnim)
      {
        ...
        // Get css value $treeAnimationDuration.  we need this because if the value is zero and
        // we start a transition, the transition end event does not fire and we can't emit the
        // expand event.
         var dur  = skipAnim? 0 : this._animDuration,
             t    = this;
    ...
    _select : function (node, check, e)
    {
             ...
             case (! isSelected && !isMultiple): 
                 if (e) {                                         // if mouse click
                   ...
                   if (this._data.core.selectMode === 1)  {
                     this._deselect(this._data.ui.selected) ; 
                 }
       ...
       if (proceed && !isSelected)  {
          ...
          this._maintainSelectionOption(node, true) ;              // maintain the options list of selections
//        this._emitEvent({ "obj" : node, "e" : e }, "select");   // now replaced by optionChange

          if (! this._data.core.suppressSelectEvent) {
            this._fireOptionChangeEvent("selection", prevSelections, this.options["selection"], e) ; 
          }
        }
     },
...
//       this._emitEvent({ "obj" : node }, "deselect");
       ...
       this._maintainSelectionOption(node, false) ;      // ensure options list of selections is correct
     ...
     _deselectAll : function(context, bSeparate)
     {
       ...
       if (! bSeparate) {
         ...
         $.each(ret, function(i, n) {
                        _this._maintainSelectionOption($(n), false) ;
                     }) ;
...
//       }
         ...
         var _this = this ;
         $.each(ret, function() {
            _this._deselect(this) ;      // deselect (also downdates the option selection array)
         }) ;
       }
     },
     ...
     /**  Maintain the options selections property
       *  @param {Object} nodes jQuery wrapped node(s) or some kind of node ref(s)
       *  @param {boolean} bAdd if true add nodes to selections, else remove from selections.
       *  @private
       */
     _maintainSelectionOption : function (nodes, bAdd)
     {
        var  ar, sels, i, j, k, $el, id, len, len2, inList ;
        var  _this = this ;
        var  bChange = false ;

        if (nodes) {
          ar   = this.options["selection"].slice(0) ;    // working copy
          sels = this.options["selection"] ;
          len  = sels.length ;

          $.each(nodes, function (i, val) {
             inList = false ;
             $el = _this._getNodeElem(val) ;         // check for valid node reference 
             if ($el) {
               id  =  $el.attr("id") ;
               for (j = 0; j < len; j++)  {         //  see if node is in current selections
                  if (id == $(sels[j]).attr("id")) {
                    inList = true ;
                    break ;
                  }
               }

               if (bAdd) {                          // if adding and not in list
                 if (! inList) {                    // add it to copy.
                   ar.push(val) ;
                   bChange = true ;
                 }
               }
               else {                               // if removing and is in list
                 if (inList)  {                     // remove it from copy.
                   len2 = ar.length ;
                   for (k = 0; k < len2; k++)  {
                      if (id == $(ar[k]).attr("id")) {
                        ar.splice(k, 1) ;
         ...
         if (s_call) {
          ...
          if (d)  {
            if (bTree)  {
              ...
              obj.append(d).children("a.oj-tree-loading").removeClass(OJT_LOADING);
...
//                       tmp.attr(m["attr"]);                // apply attr's to the <a>
                      ...
                      if (! bIns) {
                        ...
                        tmp.prepend("<ins class='oj-tree-icon " + OJT_NICON + " oj-component-icon" + "'>&#160;</ins>", sp);    // node icon
                        bIns = true ;
                      }
                      if (!m["icon"] && js["icon"])  {
                        m["icon"] = js["icon"];
           ...
           d.append(tmp);            // append the <a> and its children to the <li> 
...
//   this._data.dnd.targ_ml_width = this._data.dnd.targ_ml_width < 25? 25   : this._data.dnd.targ_ml_width ;
     ...
     /*
      *   A node has been moused over while dragging.
      *   @private
      */
...
//                          }, "<ins class='oj-tree-icon'></ins>" + dt);        // we no longer use an icon
     ...
     {  
         ...
         ctl.user_data = data;
         ctl.is_down   = true;
         ctl.helper    = $("<div class='oj-tree-title oj-drag' style='top:-2000px' />").html(html); //.fadeTo(10,0.25);
...
//      this._initCrrm() ;          // not in V1
     ...
     /**
       ...
       *   @param {string} eventname the raw event name (e.g. "hover", or "expandAll".
     ...
     _fireOptionChangeEvent : function(key, prevVal, newVal, origEvent)
     {
       ...
       var  i, j, id, len, len2, inList ;
       var  bFire = (prevVal.length != newVal.length) ; 

       if (! bFire) {
         len  = prevVal.length  ;                   // arrays have same length

         for (i = 0; i < len; i++) {
            id = $(prevVal[i]).attr("id") ;
            inList = false ;
            for (j = 0; j < len; j++) {
               if (id == $(newVal[j]).attr("id")) {
                 inList = true ;
                 break ;
               }
            }

            if (! inList) {                        // if not in list fire event
              bFire = true ;
              break ;
            }
         }
...
//Wed3                      if(this.parentNode)
     ...
     _initDnD : function()
     {
        ...
        vars.m =                                 // dnd insertion point marker
         $("<div class='oj-tree-drop-marker'><span class='oj-tree-drop-ptr oj-component-icon'>&#160;</span></div>")              // dnd insertion point marker
            .hide()
...
//     }
     ...
     _initData : function()
     {
        ...
        data.ui =  {
                      ...
                      animDurDiv        : null        // div used to et animation duration
                   };
     ...
     _keyHandler : {
         ...
         { 
           ...
           if (hover)  {
             ...
             this._fireOptionChangeEvent("selection", prevSelections, this.options["selection"], event) ; 
           } 
...
//Fri2      h2.width(Math.min(h1.text("pW" + h2[0].value).width(),w))[0].select();
     ...
     /**
       *  Get the duration from the css $treeAnimationDuration value.  
       *  @private
       */
