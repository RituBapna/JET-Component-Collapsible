 ...
 * <code class="prettyprint">aria-label</code> to the toolbar element.  The <code class="prettyprint">aria-label</code> is optional 
 * when there is only one toolbar.
 * 
 * <!-- TODO: REVIEW WHETHER ANY PART OF THE FOLLOWING BUTTONSET VERBIAGE WAS RELEVANT TO TOOLBAR, AND UPDATE ACCORDINGLY
 * 
 * <p>In addition, as shown in the online demos, the application is responsible for applying 
 * <code class="prettyprint">aria-controls</code> attributes like the following to the toolbar element, if applicable per the instructions that follow:
 * 
 * <pre class="prettyprint">
 * <code>aria-controls="myTextEditor"
 * </code></pre>
 * 
 * <p>The <code class="prettyprint">aria-controls</code> attribute is appropriate if the toolbar is controlling something else on the page, e.g. 
