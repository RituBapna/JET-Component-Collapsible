 ...
 */
  ...
  */
  ...
  /**
   * coerce. Throw an error if value is NaN, and return the coerced value.
   * @private
   */
  _coerceInputNumberValue: function(val)
  {
    ...
    if (isNaN(returnValue))
     // We don't blow up for invalid values. Log an error and move on.
     // e.g., if options.value is set to "abc", component code logs error and returns NaN for
     // ojInputNumber("option", "value");  
     oj.Logger.error("ojInputNumber's value is not a number: " + val);
